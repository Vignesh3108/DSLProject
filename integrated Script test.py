...
TrainMetaData.to_csv('Results/TestMetaData.csv')
...
images = os.listdir('Data/test_images/')
images = [i for i in images if 'jpg' in i]
imgs = Parallel(n_jobs=-1)(delayed(ImageLoad)('Data/test_images/' + i) for i in images)
...
############################################################################################################################
Data_in = pd.read_csv('Data/test.csv')
Data1 = Data_in[[ 'Age', 'MaturitySize', 'FurLength', 'Quantity', 'Fee', 'PetID', 'PhotoAmt', 'VideoAmt',]]
...
model.fit([img, X_Train], Y_train, validation_split=0.2, batch_size=300, epochs=30)
