...
def load_image(filename, im_sz=224):
    ...
    new_image = cv2.resize(imageee,(im_sz,im_sz))
    return new_image
...
Data_in.info()
Data_in.drop(['Key', 'PetId', 'PNo' , 'PetID'], axis=1, inplace=True)
...
inputs = Input(shape=(386,))
y = Dense(386, activation='relu')(inputs)
y = Dropout(0.4)(y)
...
model.fit([img, X_Train], Y_train, validation_split=0.2, batch_size=100, epochs=10, callbacks = [early_stopping_monitor])

for layer in model.layers:
    layer.trainable = False
    

for layer in model.layers[292:312]:
