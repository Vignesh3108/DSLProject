...
images = os.listdir('Data/train_images/')
images = [i for i in images if 'jpg' in i]
imgs = Parallel(n_jobs=-1)(delayed(ImageLoad)('Data/train_images/' + i) for i in images)
...
Data_in.info()
Data_in.drop(['Key', 'PetId', 'PNo' , 'PetID_x'], axis=1, inplace=True)
...
y = Dense(77, activation='relu')(inputs)
...
# model.fit([img, X_Train], Y_train, validation_split=0.2, batch_size=1000, epochs=25, callbacks=[early_stopping_monitor])
